...
void main() async {
  ...
  group('String-Collection Converters', () {
    test('List<int> to String', () {
      final source = [1, 2, 3];
      expect(service.convert<String>(source, Class.of<String>()), '1, 2, 3');
    });
    ...
    test('Set<int> to String', () {
      final source = {1, 2, 3};
      expect(service.convert<String>(source, Class.of<String>()), anyOf('1, 2, 3', '2, 1, 3', '3, 1, 2', '1, 3, 2', '2, 3, 1', '3, 2, 1')); // Order not guaranteed
    });

    test('String to Set<int>', () {
      final source = '1, 2, 3';
      final targetType = Class.of<Set<int>>();
      final result = service.convertWithClass(source, targetType);
      expect(result, {1, 2, 3});
  ...
  group('Object-Collection Converters', () {
    test('Object to List<int> (single element conversion)', () {
      final source = '123';
      final targetType = Class.of<List<int>>();
      final result = service.convertWithClass(source, targetType);
      expect(result, [123]);
    ...
    test('Object to Set<int> (single element conversion)', () {
      final source = '456';
      final targetType = Class.of<Set<int>>();
      final result = service.convertWithClass(source, targetType);
      expect(result, {456});
    ...
    test('List<int> to Object (extract single element)', () {
      final source = [789];
      final targetType = Class.of<int>();
      final result = service.convertWithClass(source, targetType);
      expect(result, 789);
    });

    test('List<int> to Object (multiple elements should throw)', () {
      final source = [1, 2];
      final targetType = Class.of<int>();
      expect(() => service.convertWithClass(source, targetType), throwsA(isA<ConversionFailedException>()));
    });

    test('Set<int> to Object (extract single element)', () {
      final source = {101};
      final targetType = Class.of<int>();
      final result = service.convertWithClass(source, targetType);
      expect(result, 101);
    ...
    test('Set<int> to Object (multiple elements should throw)', () {
      final source = {1, 2};
      final targetType = Class.of<int>();
      expect(() => service.convertWithClass(source, targetType), throwsA(isA<ConversionFailedException>()));
    });
